...
export default function InfracoesObras({ selecionadas = [], onChange, mostrarValores = false }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
        {['q61', 'q62'].map(q => (
          <button key={q} onClick={() => setQuadro(q)} style={{
            flex: 1, padding: '8px', borderRadius: '8px',
            border: `2px solid ${quadro === q ? '#1A56DB' : '#E2E8F0'}`,
        ...
        {filtradas.map(inf => {
          ...
          return (
            <button key={inf.id} onClick={() => toggle(inf)} style={{
              ...
              border: `2px solid ${sel ? '#1A56DB' : '#E2E8F0'}`,
              ...
              <div style={{
                ...
                border: `2px solid ${sel ? '#1A56DB' : '#CBD5E0'}`,
                background: sel ? '#1A56DB' : '#fff',
